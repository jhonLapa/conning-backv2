...
namespace Infraestructure.Repositories
{
    public class RegimenPrevisionalRespositorio : CrudCoreRespository<RegimenPrevisional, int>, IRegimenPrevisionalRepositorio
    {
        ...
        public RegimenPrevisionalRespositorio(ApplicationDbContext context) : base(context) => _context = context;

        public async Task<PaginadoResponse<RegimenPrevisional>> BusquedaPaginado(PaginationRequest dto)
        {
            var contex = _context.Set<RegimenPrevisional>().AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(dto.Sort))
            {
                ...
                contex = column switch
                {
                    ...
                    "tipo" => order == "desc" ? contex.OrderByDescending(p => p.Tipo) : contex.OrderBy(p => p.Tipo),
            ...
            PaginadoResponse<RegimenPrevisional> response = new(data, meta);
