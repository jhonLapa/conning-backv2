...
{
    public class AportesEmpleadorRespositorio : CrudCoreRespository<AportesEmpleador, int>, IAportesEmpleadorRepositorio
    {
        private readonly ApplicationDbContext _context;
        public AportesEmpleadorRespositorio(ApplicationDbContext context) : base(context) => _context = context;

        public async Task<PaginadoResponse<AportesEmpleador>> BusquedaPaginado(PaginationRequest dto)
        {
            var contex = _context.Set<AportesEmpleador>().AsQueryable();
            ...
            PaginadoResponse<AportesEmpleador> response = new(data, meta);

            return response;
        }

        public async Task<IReadOnlyList<AportesEmpleador>> SelectActivo()
        {
            return await _context.Set<AportesEmpleador>()
                                 .AsNoTracking()
                                 .Where(a => a.Estado == 1)
                                 .ToListAsync();
        }
