...
{
    public class TipoComprobanteRespositorio : CrudCoreRespository<TipoComprobante, int>, ITipoComprobanteRepositorio
    {
        private readonly ApplicationDbContext _context;
        public TipoComprobanteRespositorio(ApplicationDbContext context) : base(context) => _context = context;

        public async Task<PaginadoResponse<TipoComprobante>> BusquedaPaginado(PaginationRequest dto)
        {
            var contex = _context.Set<TipoComprobante>().AsQueryable();
            ...
            if (!string.IsNullOrWhiteSpace(dto.Sort))
            {
                ...
                {
                    "codigo" => order == "desc" ? contex.OrderByDescending(p => p.Codigo) : contex.OrderBy(p => p.Codigo),
            ...
            PaginadoResponse<TipoComprobante> response = new(data, meta);
